...
#include <unistd.h>
    ...
    static int tp_parse_and_append_header_line(tp_headers *headers_parsed, const char *line, size_t linelen)
    {
        ...
        size_t name_leading_ws = tp_trim_leading_ws(name_start, raw_name_len);
        name_start += name_leading_ws;
        size_t name_len = raw_name_len - name_leading_ws;
...
#include <stdarg.h>
    ...
    int teapot_send_response(stb_teapot_socket_t client, const teapot_response *resp)
    {
        ...
        int header_len = snprintf(NULL, 0, header_fmt,
        ...
        int written = snprintf(header, header_size, header_fmt,
                               resp->status, teapot_status_to_str(resp->status), ct, tp_da_len(resp->body));
