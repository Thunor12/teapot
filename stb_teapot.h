...
#define MSG_NOSIGNAL 0
    ...
    static int teapot_parse_content_length_value(const tp_string_builder *cl_val, size_t *out_length)
    {
        size_t value = 0;
        const char *p = NULL;
        ...
        *out_length = 0;
        ...
        while (*p != '\0' && !isspace((unsigned char)*p))
        {
            ...
            if (value > (((size_t)TEAPOT_MAX_BODY_SIZE - digit) / 10u))
            {
                return -1;
            }
    ...
    static int teapot_request_content_length(const teapot_request *req, size_t *out_length)
    {
        if (req == NULL)
        {
            return -1;
        }
        return teapot_parse_content_length_value(tp_headers_get(&req->headers, "Content-Length"), out_length);
    }
        ...
        for (size_t i = 0; i < server->route_count; i++)
        {
            ...
            if (path_len > 0 && r->path[path_len - 1] == '*')
            {
                ...
                if (strncmp(r->path, req->path.items, prefix_len) == 0 &&
                    (prefix_len > 0 && r->path[prefix_len - 1] == '/'
                         ? 1
                         : (req->path.items[prefix_len] == '\0' || req->path.items[prefix_len] == '/' ||
    ...
    static void teapot_send_bad_request(stb_teapot_socket_t client)
    {
        teapot_response resp;
        teapot_response_init(&resp, TEAPOT_HTTP_BAD_REQUEST);
        tp_sb_appendf(&resp.body, "400 Bad Request\n");
    ...
    static int teapot_complete_request_body(stb_teapot_socket_t client, teapot_request *req)
    {
        ...
        if (teapot_request_content_length(req, &expected_body) != 0)
        {
            return -1;
        }
    ...
    int teapot_handle_client_connection(teapot_server *server, stb_teapot_socket_t client)
    {
        ...
        {
            free_request(&req);
            teapot_send_bad_request(client);
            teapot_close((stb_teapot_socket_t)client);
            return -1;
        }

        /* If Content-Length says we should have more body, read remaining bytes before routing. */
