...
#define tp_da_append_many(da, new_items, new_items_count)                                             \
    do                                                                                                \
    {                                                                                                 \
        size_t tp_da_new_items_count = (new_items_count);                                             \
        if (tp_da_new_items_count > 0)                                                                \
        {                                                                                             \
            tp_da_reserve((da), (da)->count + tp_da_new_items_count);                                 \
            memcpy((da)->items + (da)->count, (new_items), tp_da_new_items_count * sizeof(*(da)->items)); \
            (da)->count += tp_da_new_items_count;                                                     \
        }                                                                                             \
    } while (0)
...
#define TP_MAX_HEADER_VALUE_LEN 4096
    ...
    static int tp_parse_content_length_value(const char *s, size_t *out_length)
    {
        if (s == NULL || *s == '\0' || out_length == NULL)
        {
            return 0;
        }
    ...
    static int tp_headers_content_length(const tp_headers *headers, size_t *out_length)
    {
        if (out_length == NULL)
        {
            return 0;
        ...
        *out_length = 0;
        ...
        return tp_parse_content_length_value(value->items, out_length);
    }
...
#include <stdarg.h>
    ...
    static int teapot_content_type_is_safe(const char *content_type)
    {
    ...
    static int parse_request(char *buffer, size_t size, teapot_request *req)
    {
        ...
        if (!tp_headers_content_length(&req->headers, &content_length) || content_length > (size_t)TP_MAX_BODY_SIZE)
        {
            tp_headers_free(&req->headers);
            return -1;
        }
    ...
    int teapot_send_response(stb_teapot_socket_t client, const teapot_response *resp)
    {
        ...
        if (!teapot_content_type_is_safe(ct))
        {
            ct = "text/plain";
        }
    ...
    int teapot_handle_client_connection(teapot_server *server, stb_teapot_socket_t client)
    {
        ...
        /* If Content-Length says we should have more body, read remaining bytes (body may be in next packet) */
        {
            ...
            if (!tp_headers_content_length(&req.headers, &expected_body) || expected_body > (size_t)TP_MAX_BODY_SIZE)
            {
                free_request(&req);
                teapot_close((stb_teapot_socket_t)client);
                return -1;
