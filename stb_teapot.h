...
#include <limits.h>
    ...
    static int teapot_write_all(stb_teapot_socket_t s, const char *buf, size_t len)
    {
        ...
        while (written < len)
        {
            ...
            flags = MSG_NOSIGNAL;
#endif
            ssize_t n = send(s, buf + written, chunk, flags);
#endif
            if (n <= 0)
            {
                return -1;
    ...
    int teapot_send_response(stb_teapot_socket_t client, const teapot_response *resp)
    {
        ...
        if (resp->body.count > 0 && (resp->body.items == NULL || teapot_write_all((stb_teapot_socket_t)client, resp->body.items, resp->body.count) < 0))
        {
            return -1;
        }
        return 0;
