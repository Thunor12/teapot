...
static void test_slash_wildcard_matches_subpath(void)
{
    char response[512];
    teapot_route routes[] = {
        {TEAPOT_GET, "/api/*", recording_handler},
    };
    ...
    ok("slash wildcard request succeeds", rc == 0);
    ok("slash wildcard reaches handler", handler_called == 1);
    ok("slash wildcard response is 200", strstr(response, "HTTP/1.1 200 OK") != NULL);
}
...
int main(void)
{
    ...
    test_slash_wildcard_matches_subpath();
